...
| 8 | 轻量／秒启动／低占用／大文件 | ⚠️ 启动首屏 −78%；解析已线性。**但约 850 KB 的文档会让渲染层饱和 >105 秒**（第 48 轮 CI 实测），打开大文档的二次方**已修复**(引用定义改为按文档版本收集):真实窗口 210 KB 渲染由 13.9 s 降到约 **3 s**;打字由 172 降到 **69 ms/击键**(约 2.5 倍)。**仍不够好**,未完成 |
...
合计约 **2.5 倍**。`deepClone` 在 profile 里由 12.9% 降到 **5.3%**,
...
2. `performance.memory` **只统计 JS 堆**,不含 DOM 节点(C++ 对象)和渲染进程 RSS。
   载入 210 KB 后堆没涨,不代表整体内存没涨——**这个数字不能拿来断言"占用低"**,
   它只能说明 JS 侧没有在囤副本。要量真实占用需要进程级 RSS,当前 E2E 拿不到。
