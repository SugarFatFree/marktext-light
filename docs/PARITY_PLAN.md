...
| 8 | 轻量／秒启动／低占用／大文件 | ⚠️ 启动首屏 −78%；解析已线性。**但约 850 KB 的文档会让渲染层饱和 >105 秒**（第 48 轮 CI 实测），嫌疑是每次变更两次全文档 `deepClone`。**这条要求目前不成立** |
...
**约 850 KB(1200 小节)的文档,在 CI 里让渲染层饱和超过 105 秒**——用例自己把超时抬到
105 秒仍然超时,`console.log` 都没轮到执行,连 `page.evaluate` 都排不进主线程,
`afterAll` 关窗也超时。这不是"慢一点",是**那段时间里编辑器不响应**。

要说清证据边界:**这是一次 CI 观测,不是 profiling**。但代码里有个很强的嫌疑——

- muya `state/index.ts` 的 `dispatch()`:`prevDoc` 与 `doc` **各做一次全文档 `deepClone`**。
- 渲染层 `json-change` 处理器每次还会再调 `getMarkdown()` / `getHistory()` / `getTOC()` /
  `getState()`(又一次 `deepClone`)。

即**每次变更都是 O(整篇文档) 的工作量**,而载入本身也要走 dispatch。
解析已经是线性的了(单测量过 2 MB→3 s),所以瓶颈不在解析,而在这一段。

E2E 里的规模**暂降到 300 小节(192 KB)**作为能跑完的护栏,并把真实耗时打印出来。
**这不是把要求 #8 缩水**——850 KB 的限制如实记在这里,是下一步的首要目标。
