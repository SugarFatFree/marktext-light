...
| 8 | 轻量／秒启动／低占用／大文件 | ⚠️ 启动首屏 −78%；解析已线性。**但约 850 KB 的文档会让渲染层饱和 >105 秒**（第 48 轮 CI 实测），打开大文档的二次方**已修复**(引用定义改为按文档版本收集):真实窗口 210 KB 由 13.9 s 降到 **3.2 s**。**打字仍约 180 ms/击键**,是另一条路径,未完成 |
...
剩下几项都有真实用途,要改得先想清楚语义(保存/脏标记依赖 markdown),不能一味 debounce。
