...
| 8 | 轻量／秒启动／低占用／大文件 | ⚠️ 启动首屏 −78%；解析已线性。**但约 850 KB 的文档会让渲染层饱和 >105 秒**（第 48 轮 CI 实测），瓶颈已定位在 `Muya.init()` 建块树、且只对容器块超线性(列表最糟)。**这条要求目前不成立** |
...
**下一步就是继续往里挖**:已排除的三条不必重查;要找的是"每建一个容器块就要走一遍
整篇文档"的那个动作。
