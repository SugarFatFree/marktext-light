...
### 保存不是原子的,以及两个静默的桩(第 116 轮)

沿用上一轮那个透镜——「Electron 主进程做了什么,Tauri 桥没做」——又挖出三件。

**1. 保存会丢文档(真数据风险)。** Tauri 侧是裸的 `fs::write`:先截断再写入。
两步之间文件是空的,两步之后字节可能还在页缓存里。**前一个窗口崩溃 → 文档被截断;
后一个窗口断电 → 得到一个等长的全零文件**,后者更糟,因为它看起来像个文件。

上游两样都踩过:`main/filesystem/index.ts` 用 `write-file-atomic`,注释里点名了
**#3786、#3828**(持久性)和 **#3509**(文档所在文件夹被删后保存失败)。
改成同样的过程:目标同目录下的唯一命名临时文件 → **rename 之前** `sync_all` → rename。
两个细节是照抄而非简化:**保留目标权限位**、**沿符号链接写到目标**(否则链接会被
替换成普通文件)。Unix 下还 fsync 父目录——只落在页缓存里的 rename 同样不持久。
7 条 Rust 单测,CI 通过。

**2. 图片上传桩会往文档里写 `[object Object]`。** 桩是 `async() => ({})`,
而 `editor.vue` 把返回值直接当图片路径用。选了「插入方式:上传」的用户粘贴图片,
文档里就多一个 URL 是 `[object Object]` 的 `![]()`,**并且存进磁盘,全程无提示**。
紧挨着那行调用就有 `catch`(提示 + 落回保存到本地文件夹)——**因为桩是 resolve,
它永远触发不了**。改成 reject 一条已翻译的消息,既有的降级路径就通了。
实现各家图床是功能,诚实地失败不是。

**3. `fonts.list` 返回空数组**,这个**保持原样但写清了理由**:WebView 枚举不了
系统字体,而从字体**文件名**推家族名会造出不存在的名字(`msyh.ttc` 是"微软雅黑")。
选择框仍有内置字体且本来就是自由输入,所以字体能填、只是不能选。

**顺带修掉一个我自己造成的 CI 缺口**:`lint` / `test` / `e2e` 只在 `pull_request`
触发,而这几轮都是直推 `develop`——**于是 CI 上只有 Rust 被验过,e2e 一次没跑**。
三条都加上 `push: branches: [develop]`。

### 用户在等结果的失败,一声不吭(第 117 轮)

Electron 侧这些失败都由主进程发 `mt::show-notification`。桥继承了动作、没继承通知,
形态每次都一样:**`catch` 里写一行没人开着的 console,然后 return**。
对话框关掉、文件还叫原来的名字、侧栏纹丝不动——**用户无从判断那一下点击有没有生效**。

逐个查出来五处,全部是用户主动操作后在等结果:

| 位置 | 用户做了什么 | 失败时发生了什么 |
|---|---|---|
| `export.ts` 写文件 | 选好路径导出 HTML | 磁盘满/只读目录 → **看起来和成功一样** |
| `export.ts` 无内容 | 导出 | 静默返回 |
| `files.ts` 重命名 | 输入新名、确定 | Windows 保留名/非法字符 → 文件名没变,无解释 |
| `files.ts` 移动到 | 选好目标 | 同上 |
| `import.ts` pandoc | 选了个 .docx | 转换失败 → 什么都没发生 |
| `project.ts` 扫描 | 打开文件夹 | 权限/网络盘断开 → 侧栏毫无反应 |

`import.ts` 那处尤其说明问题:**函数的文档注释写着"or explain why it could not be
done"**,而且紧邻的 `reportMissingPandoc` 处理了"pandoc 不存在",
**唯独"转换失败"这一支是空的**。

**修法不是逐个补,而是加一条结构性守卫**:`bridge-silent-failures.spec.ts`
遍历桥里每一处 `console.error`,要求它所在的块**要么通知用户,要么在白名单里写明
为什么该沉默**。白名单按 `文件:片段` 作键(同样的措辞在不同文件里含义不同),
并且有一条测试专门查**过期的豁免**——豁免活得比它的站点久,正是下一处被放行的方式。

**这条守卫当场抓到了我自己的疏漏**:`export.ts` 那两处我已经通知了,但调的是
`notifyExportFailed`,不在我第一版按名字列的匹配里。改成按**形状**匹配
(`notify[A-Z]\w*\(`)。**第一版列了三个名字就漏掉了第四个正在做正确事情的**——
这本身就是这条测试该按形状而非名单来写的理由。
...
### 行长这个维度此前完全没测过,查出链接规则是超线性的(第 120 轮)

现有的线性度 e2e 变的是**块数**,行都很短。**单行长度是另一种形状**,而且是拖垮编辑器的
经典方式(粘进来一坨压缩 JSON、一个 base64 图、一条日志)。此前**一条测试都没有**。

按标记类型分别测(8 / 32 / 128 KB,单行,空闲机器,ms/KB):

| 输入 | 8 KB | 32 KB | 128 KB | |
|---|---|---|---|---|
| 纯文本 | 1.12 | 1.06 | 1.04 | 平 |
| `*a*` | 2.77 | 2.99 | 2.42 | 平 |
| `_b_` | 2.25 | 2.32 | 2.38 | 平 |
| `` `c` `` | 0.39 | 0.70 | 0.40 | 平 |
| `~~f~~` | 0.46 | 0.45 | 0.46 | 平 |
| **`[d](e)`** | **1.64** | **3.31** | **8.59** | **不平** |

块级 lexer 全程可忽略(0.008–0.024 ms/KB),**问题全在 inline**,且**只在链接**。

**原因**:`link` 规则的目标部分是贪婪的 `(.*)`,会一路吃到行尾再回溯找 `)`。
每个链接 O(行长) → 整行 O(n²)。**128 KB 的一行链接要 1.1 秒。**

**先验了正确性,不是猜的**:`[a](b) and (c)` 仍解析出 href `b`,
`[a](b)(c)`、`![i](p.png) and (note)` 也都正确。**贪婪会回溯到对的 `)`,
所以这不是正确性 bug,纯粹是回溯成本。**

**本轮不改这条正则。** 改核心解析器的链接规则,唯一的判据是 670 条一致性用例,
这该占一整个专门会话,而不是定时巡检里的一个槽。
`lineLengthLinearity.spec.ts` 把其余规则锁成"平",
链接那条给一个**刚好容得下现状、容不下更差**的上限(8→64 KB 实测 2.0,阈值 3.5)——
**不是许可证,是天花板**:重写链接规则的人有个数字要打败,别人也别想把它推得更高。

**注意**:muya 包用的是 antfu 配置(分号、4 空格),和 desktop 包不同。新测试按所在包的规矩写。

### 设置窗口改的偏好,编辑器窗口收不到(第 121 轮)

Electron 的主进程**拥有**偏好文件:设置窗口请它写,它写完向**每个窗口**推
`mt::user-preference`,于是打开着的编辑器跟着变。Tauri 下每个窗口自己写文件,
而桥**只写文件**——

**主题、字体、行高、缩进宽度、拼写检查、侧栏行为,全部落盘后屏幕上毫无变化,要重启才生效。**

`language` 是唯一的例外,因为有人单独给它加了 `broadcastLanguage`,**但没人把它一般化**。
`pickFolderPreference` 也是本地推送——在设置里换图片文件夹,编辑器仍往旧的存粘贴的图。

**关键区分是「本地」还是「全局」**:
回答 `mt::ask-for-user-preference` 是答复**发问的那个窗口**,理应本地;
记录一次**变更**是对所有窗口的通知,必须走 Tauri 的 `emit`。
`emit` 会送达包括发送者在内的每个窗口,而 `registerEvent` 已经把每个窗口的 store
订阅上去了——**所以发送者不需要再本地派发一次,否则同一份 patch 会到两遍**。
...
### 快捷键设置页是一张空表配两个死按钮(第 122 轮)

沿着上一轮那条跨窗口的线继续,查设置窗口的快捷键页。

`onMounted` 里向两个通道要数据,**并在 `.then` 的形参里解构**:
...
Tauri 下未实现的 invoke **resolve 成 `undefined`**(桥里有意为之,见 `routedInvoke`),
解构 `undefined` 抛 TypeError,**正好掉进那个只写日志的 `.catch`**。
于是 `keybindingList` 永远是 `[]`——

**页面是一张空表,配着「保存」和「恢复默认」两个按不出反应的按钮,没有任何解释。**

这正是本项目一直在避免的「静默地坏掉」:对照之下自动更新是**干净地缺席**
(命令面板项被 `isUpdatable()` 挡住、菜单里根本没有条目),**没有死 UI**。

**两半都要,缺一不可**:

1. **桥提供真实的默认表**(`getPrefKeybindings`),页面于是列出**当前真正生效**的快捷键
   ——这本来就是这个页面大部分的价值。`userKeybindings` **恒为空**:
   页面把这张表里的条目标记为"已自定义",而现在没有任何东西能兑现一个自定义绑定,
   **谎报比不报更糟**。
2. **编辑控件全部禁用并写明原因**。保存一个绑定意味着写一个 Electron `Keybindings`
   类拥有的文件,再让渲染层的快捷键层**和原生菜单**都去兑现它——Tauri 侧一样都还没有。
   只做第 1 步会得到一个"能改、能存、但毫无效果"的页面,**比空表更坏**。
...
### 三次扫描全空:机械排查已经收敛(第 125 轮)

**没有找到新问题。** 记下来是为了让下一次不要重走。

**扫描一:定时器。** 组件里把定时器句柄存进变量(说明打算取消它)却不清的——**两处,都没问题**。
`app.vue` 是永不卸载的外壳;`textBox` 的防抖在处理函数里自清,
卸载后仍会触发一次并写入用户刚敲的偏好——**那是想要的行为,不是泄漏**。
注意上一次(第 119 轮)我只比对了 `setInterval`/`clearInterval`,
**漏掉了 `setTimeout` 自调度这种轮询**;这次补上了,图床那个 PicGo 轮询清理得很干净。

**扫描二:桥以外的静默失败。** 30 处只写日志的 catch,逐条按"用户是否在等结果"筛,
**没有一处够得上上一轮 `search.vue` 那种**。命令面板两处用
`throw new Error(null)` 当取消信号(有注释),后台缓冲状态、CodeMirror 语法模式
都不是用户在等的东西。

**扫描三(其实是两个被否掉的假设):**

1. **主题加载失败会留下"新深浅 class + 旧配色"的破碎状态** —— 成立,但只可能由
   本地资源 chunk 取不到触发,概率极低。
2. **`loadThemeCss` 并行加载的 Prism CSS 没有存在性检查** —— 看着像:
   `hasThemeStylesheet` 只检查编辑器那份,缺 Prism 会让 `Promise.all` 整体 reject,
   **连存在的编辑器 CSS 也一起丢掉**。**但 `load()` 自己就处理了缺失的键**
   (记录并返回空串),所以最坏只是没有语法高亮。**不成立。**

**结论:机械排查这条路已经收敛。** 通道三个方向穷尽(第 123–124 轮)、
监听器与定时器干净(第 119、125 轮)、静默失败只剩不值得动的残余。
剩下的都不是"再扫一遍就能找到"的东西——见下面「下一步」与「只有用户能做的」两节。

### 三个新透镜也全空,连续第二轮(第 126 轮)

换了三个和之前完全不同的角度,**都没有问题**。

**一、语言文件的"值"有没有真的翻译**(键的齐整已有 `locale-parity.spec.ts` 管,
但**值**没人管)。各语言与英文完全相同的条目占 4–8%。
用**中文那 41 条**做基准逐条判断——全部正当:主题专名(Dracula、Nord)、纸张尺寸、
**语言自称**(用各自语言显示是正确做法)、安装命令、产品名。**中文是真的翻译完整的。**
再拿中文基准去减其余语言,剩下的也基本正当:`common.ok`='OK'、
法语的 `Image`/`Page`/`Style`/`Format`/`Options` **本来就是同一个词**。
只有 es/pt 的 `Super/Sub script` 算真没译。
**没有动手改法语德语西语的译文——凭猜测改是把事情变糟的好办法。**

**二、中文主题名一半译一半没译?** 看着像不一致,实际**有章法**:
品牌名保留(Ayu、Gruvbox、Solarized),只翻译变体限定词(Dark → 深色);
没有限定词的整名保留(Dracula、Nord、Kanagawa);
`One Dark`、`Tokyo Night` 是完整品牌名,正确地整体保留。**不是缺陷。**

**三、图片路径里的空格与 `#` 转义** —— `packages/muya` 里已有
`imageSrcEscapeParity.spec.ts` 专门管这件事。

## 机械排查到此为止

连续两轮空(第 125、126 轮),而且这两轮试的都是**新角度**而非重复旧扫描。
从"静默失败 / 通道缺口 / 资源泄漏 / i18n"这几个透镜能挖到的,已经挖完了。

**剩下的三类,没有一类是"再扫一遍"能解决的**:
- **要用户决定**:自动更新的托管位置与签名密钥
- **要单开会话**:inline 链接规则的 O(n²)(已被上限锁住)、
  用户自定义快捷键的三段实现、窗口背景色由 Rust 在建窗口前定
- **要用户的眼睛**:深色模式与 CSS 裁剪后的布局目视确认
...
3. **用户自定义快捷键**（第 122 轮）：桥现在提供默认表、编辑控件已禁用并写明原因，
   所以**没有死 UI**。要真正支持，需要三件：一个存 `keybindings.json` 的地方、
   渲染层 `shortcuts.ts` 读它、Rust 菜单的 accelerator 也读它（`menu/mod.rs`
   注释里"Not yet ported"的那条）。是功能，不是 bug。
4. **拼写检查**：**这条的「5 个通道硬缺口」说法已过时**。逐条查证后只有
...
- **重写 inline 链接规则**（第 120 轮测出）：目标部分的贪婪 `(.*)` 让一行链接变成
  O(n²)，128 KB 一行要 1.1 秒。改它就是改核心解析器，**唯一判据是 670 条一致性用例**，
  要单开会话。现状已被 `lineLengthLinearity.spec.ts` 的上限锁住，不会再变差。
