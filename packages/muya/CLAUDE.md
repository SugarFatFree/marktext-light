...
- `pnpm -C packages/muya/e2e e2e` — Playwright E2E (chromium/firefox/webkit). `e2e:install` is a one-time browser install. CI (`muya.yml`) runs Chromium only; Firefox + WebKit are configured in `playwright.config.ts` and runnable locally, but excluded from the CI matrix until the engine-independent rewrites in BACKLOG Phase 3 land (triple-click selection, search-replace mutation timing).
...
- **Reference link/image definitions** (`[ref]: url "title"`) are NOT a first-class block type in state. `markdownToState`'s `case 'def'` re-emits the raw definition line back into a `paragraph` state node so it round-trips losslessly through the markdown serializer. `InlineRenderer` populates a labels Map that the lexer consults when expanding `[text][ref]` and `![alt][ref]`. It is collected **once per document version**, not per render pass: `patch` runs for every content block, and re-collecting there deep-cloned and walked the whole document each time, making a document open in O(blocks²). The cache is keyed on `JSONState.version` rather than a `json-change` subscription — replacing the content rebuilds and repaints the tree *before* that event is emitted, so an event-driven cache would serve the outgoing document's definitions for exactly the repaint that needed the new ones. `ILinkReferenceDefinitionState` exists as a deprecated stub for compatibility — do not introduce new code paths that produce it.
